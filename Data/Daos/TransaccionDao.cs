...
namespace AriesWebApi.Data.Daos
{
...

    {
        ...
        /// <returns></returns>
        public Transaccion Insert(Transaccion transaccion, double idAsiento, Usuario user)
        {
            using (MySqlTransaction tr = manejador.GetConnection().BeginTransaction(IsolationLevel.Serializable))
            {
                try
                {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(sql, tr.Connection, tr))
                    {
                        ...
                        transaccion.Id = Convert.ToDouble(id);
                    }
                    tr.Commit();
                    return transaccion;
                }
        ...
        public bool Update(Transaccion tr, Usuario usuario, out String mensaje)
        {
            ...
            try
            {
                ...
                List<Parametro> lst = new List<Parametro>();
                lst.Add(new Parametro("@account_id", tr.CuentaDeAsiento.Id));
                lst.Add(new Parametro("@reference", tr.Referencia));
                lst.Add(new Parametro("@detail", tr.Detalle));
                lst.Add(new Parametro("@balance", tr.Monto));
                lst.Add(new Parametro("@balance_type", (int)tr.ComportamientoCuenta));
                lst.Add(new Parametro("@money_type", (int)tr.TipoCambio));
                lst.Add(new Parametro("@money_chance", tr.MontoTipoCambio));
                lst.Add(new Parametro("@bill_date", tr.FechaFactura));
                lst.Add(new Parametro("@updated_by", usuario.UsuarioId));
                lst.Add(new Parametro("@transaction_accounting_id", tr.Id));
        ...
        public List<Transaccion> GetCompleto(Asiento asiento)
        {
            ...
            foreach (DataRow item in dt.Rows)
            {
                ...
                Transaccion tran = new Transaccion(
                    ...
                    balance: Convert.ToDouble(vs[4]),
                    comportamientoCuenta: ((Comportamiento)Convert.ToInt32(vs[5])),
                    montoTipoCambio: Convert.ToDouble(vs[7]),
                    cuentaDeAsiento: new Cuenta
                    {
                        Nombre = Convert.ToString(vs[8]),
                        Id = Convert.ToDouble(vs[9]),
                        Padre = Convert.ToDouble(vs[10]),
        ...
        public String Delete(List<Transaccion> lst, Double idAsiento, Usuario user)
        {
